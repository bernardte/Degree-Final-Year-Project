...
const useUserStore = create<userStore>((set) => ({
  ...
  updateSuspended: (userId: string, newStatus: boolean) =>
    set((state) => ({
      user: state.user.map((u) =>
        u._id === userId ? { ...u, suspended: newStatus } : u,
      ),
    })),
  fetchUser: async (page: number, limit = 10, searchTerm = "") => {
    set({ isLoading: true, error: null });
    try {
      const response = await axiosInstance.get(
        `/api/admin/get-user?page=${page}&limit=${limit}&search=${searchTerm}`,
      );
  ...
  fetchCurrentLoginUser: async () => {
    ...
    try {
      const response = await axiosInstance.get(
        "/api/users/getCurrentLoginUser",
      );
      const user = response?.data;
      set({ currentLoginUser: user });
    } catch (error: any) {
      console.log(
        "Error in fetchCurrentLoginUser: ",
