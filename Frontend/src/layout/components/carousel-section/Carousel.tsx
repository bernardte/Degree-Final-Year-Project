...
const Carousel = ({ category }: CarouselProps) => {
  ...
  const handleButtonClick = (link: string | undefined, e: React.MouseEvent) => {
    ...
    if (link.startsWith("http")) {
      window.location.href = link; // 完整 URL 用原生跳转
    } else {
      navigate(link); // 内部路由用 navigate
    }
  };
  ...
  return (
    <section className="group relative w-full overflow-hidden">
      ...
      >
        {carousel.map((c) => (
          <div key={c._id} className="relative w-screen flex-shrink-0">
            ...
            />
            <div className="absolute inset-0 z-20 flex items-end bg-gradient-to-t from-blue-900/80 to-transparent">
      ...
      <div className="absolute top-0 z-20 flex h-full w-full items-center justify-between text-white opacity-0 group-hover:opacity-100">
      ...
      <div className="absolute bottom-0 z-20 flex w-full justify-center gap-3 py-4">
